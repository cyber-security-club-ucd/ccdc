...
# FIX: Helper to safely get a command path without using ?. operator (PS 5.1 compat)
...
# FIX: Replaced ?.Source null-conditional with helper function (PS 5.1 compat)
if (-not $phpExe) {
    $phpExe = Get-CommandPath "php.exe"
}

if ($phpExe) {
    Write-INFO "PHP found: $phpExe"

    # FIX: PHP prints extension-load warnings to stdout which pollute the captured string.
    # Redirect stderr to null, then strip any remaining Warning: lines from stdout,
    # and grab only the last non-empty token which is the actual version/path value.
    $phpRawVersion = & $phpExe -r "echo phpversion();" 2>$null
    $phpVersion = ($phpRawVersion -split "`n" | Where-Object { $_ -notmatch "^\s*Warning:" -and $_.Trim() -ne "" } | Select-Object -Last 1).Trim()
    Check-Version "PHP" $phpVersion

    $phpRawIni = & $phpExe -r "echo php_ini_loaded_file();" 2>$null
    $phpIniPath = ($phpRawIni -split "`n" | Where-Object { $_ -notmatch "^\s*Warning:" -and $_.Trim() -ne "" } | Select-Object -Last 1).Trim()
    Write-INFO "php.ini: $phpIniPath"

    if ($phpIniPath -and $phpIniPath -ne "" -and (Test-Path $phpIniPath)) {
        ...
        foreach ($check in $phpChecks) {
            $val   = $check.Current
            $color = "Green"
            # FIX: Replaced Unicode arrow with plain ASCII
            $flag  = ""
...
# FIX: Replaced ?.Source null-conditional with helper function (PS 5.1 compat)
if (-not $mysqlExe) {
    $mysqlExe = Get-CommandPath "mysql.exe"
}
...
if ($sshdService) {
    ...
    if (Test-Path $sshdConfig) {
        ...
        foreach ($check in $sshChecks) {
            ...
            # FIX: Replaced Unicode em-dash in string with plain ASCII --
            if ($m.Success) {
                $current = $m.Groups[1].Value
            } else {
                $current = "(not set -- default applies)"
            }
            $isSafe = ($m.Success -and $current -eq $check.Safe) -or
                      (-not $m.Success -and $check.Setting -eq "PasswordAuthentication")
            $color = if ($isSafe) { "Green" } else { "Yellow" }
            Write-Host ("  {0,-30} Current: {1,-20} Recommended: {2}" -f $check.Setting, $current, $check.Safe) -ForegroundColor $color
        }
    }
} else {
...
Write-INFO "Searching for config files..."
...
# ---------------------------------------------------------------------------
# 6. Unknown Process / Service Inventory
# ---------------------------------------------------------------------------
Write-STEP "Unknown Process / Service Inventory"

$knownProcesses = @(
    "svchost","lsass","csrss","wininit","services","smss","winlogon",
...
foreach ($p in $processes | Sort-Object Name) {
    ...
    # FIX: Replaced ?.Status null-conditional with explicit null check (PS 5.1 compat)
    $sigObj = if ($exe -ne "Access Denied") { Get-AuthenticodeSignature $exe -ErrorAction SilentlyContinue } else { $null }
    $sig    = if ($sigObj) { $sigObj.Status } else { "Unknown" }
