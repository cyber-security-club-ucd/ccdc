...
sql_test(){
    ...
    if [ -n "$SQLDIR" ]; then
        SQLCONFINFO=$( DPRINT find "$SQLDIR" -type f \( -name '*sql*.cnf' -o -name '*-server.cnf' \) -exec grep -HnE 'user\s*=|bind-address\s*=' {} + | sed 's/:user\s*/ ===> user /' | sed 's/bind-address\s*/ ===> bind-address /' )
    fi
